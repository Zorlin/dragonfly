...
/// Checks if the Kubernetes API server is reachable by attempting to create a client.
pub async fn check_kubernetes_connectivity() -> Result<()> {
    debug!("Attempting to connect to Kubernetes API server...");
    Client::try_default().await.wrap_err("Failed to connect to Kubernetes API server. Is k3s running and KUBECONFIG configured?")?;
    debug!("Successfully connected to Kubernetes API server.");
    Ok(())
}
